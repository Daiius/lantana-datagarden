...
# 開発技術検討
このプロジェクトに限らない汎用的な性質として、
どのライブラリやフレームワークを使用するべきかちょっと検討
- これまで開発or開発予定のWebアプリケーション例
  - 軽量SPAジョークアプリ
  - Todoアプリ
  - キャラクター人気投票アプリ
  - 動画配信コントロールアプリ
...
他ユーザの変更を出来るだけ早く画面に反映したい等、
リアルタイム性が要求される場合や、
ユーザのリクエストとは異なる任意のタイミングで処理をする場合には
専用サーバが必要になる

もう一つ、データの一方通行性について何か軸を作れそうな気もする
初回レンダリングの速度や情報量が必要ならばNext.jsだし、
要らなければSPAで良いみたいなあるだろうか？


```mermaid
quadrantChart
  title アプリ要件とおすすめ設計
  x-axis "リクエスト時に処理完結" --> "任意タイミングでの処理必須"
  y-axis "無し / S→C一方通行" --> "S↔C双方向"
  quadrant-1 "Backend server + Vite"
  quadrant-2 "Next.js with Server Actions"
  quadrant-3 "Vite + React SPA"
  quadrant-4 "Backend server + Next.js"
  To-Dog List : [0.1, 0.1]
  Sekirei Todo : [0.1, 0.8]
  Girl's Side Analysis : [0.4, 0.7]
  Cephylas : [0.9, 0.1]
  Videmus : [0.9, 0.9]
  Lantana Datagarden : [0.7,0.7]
```

Videmusの入っている位置がちょっと違うかも......
